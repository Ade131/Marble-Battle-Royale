  ...
  [DisallowMultipleComponent]
    ...
    public class FusionBootstrap : Behaviour
    {
      ...
      public enum StartModes
        ...
        /// </summary>
        [InlineHelp]
        [WarnIf(nameof(RunnerPrefab), false,
            "No " + nameof(RunnerPrefab) + " supplied. Will search for a " + nameof(NetworkRunner) +
        ...
        /// </summary>
        [InlineHelp]
        [WarnIf(nameof(StartMode), (long)StartModes.Manual,
            "Start network by calling the methods " + nameof(StartHost) + "(), " + nameof(StartServer) + "(), " +
            nameof(StartClient) + "(), " + nameof(StartHostPlusClients) + "(), or " + nameof(StartServerPlusClients) +
            "()")]
        public StartModes StartMode = StartModes.UserInterface;

        /// <summary>
        ///     When <see cref="StartMode" /> is set to <see cref="StartModes.Automatic" />, this option selects if the
        ///     <see cref="NetworkRunner" />
        ...
        /// <summary>
        ///     <see cref="FusionBootstrapDebugGUI" /> will not render GUI elements while <see cref="CurrentStage" /> ==
        ///     <see cref="Stage.AllConnected" />.
        /// </summary>
        [InlineHelp] [DrawIf(nameof(StartMode), (long)StartModes.UserInterface, Hide = true)]
        ...
        ///     If this field is null or invalid, will be set to the current scene when <see cref="FusionBootstrap" /> runs
        ///     Awake().
        /// </summary>
        [InlineHelp] [ScenePath] public string InitialScenePath;

        /// <summary>
        ///     Indicates which step of the startup process <see cref="FusionBootstrap" /> is currently in.
        /// </summary>
        [InlineHelp] [SerializeField] [ReadOnly]
        protected Stage _currentStage;

        [NonSerialized] private NetworkRunner _server;
        ...
        protected bool CanAddClients => CurrentStage == Stage.AllConnected && CurrentServerMode > 0 &&
                                        CurrentServerMode != GameMode.Shared && CurrentServerMode != GameMode.Single;

        protected bool CanAddSharedClients => CurrentStage == Stage.AllConnected && CurrentServerMode > 0 &&
                                              CurrentServerMode == GameMode.Shared;
        ...
        protected bool ShowAutoClients => UsingMultiPeerMode && (StartMode == StartModes.UserInterface ||
                                                                 (StartMode == StartModes.Automatic &&
                                                                  AutoStartAs != GameMode.Single));
        ...
        protected virtual void Start()
        {
            ...
            if (existingRunner && existingRunner != RunnerPrefab)
            {
                ...
                if (RunnerPrefab == null) RunnerPrefab = existingRunner;
            }
            ...
            switch (StartMode)
            {
                ...
                case StartModes.Automatic:
                {
                    if (TryGetSceneRef(out var sceneRef))
                    {
                        ...
                        else if (AutoStartAs == GameMode.Client || AutoStartAs == GameMode.Shared ||
                                 AutoStartAs == GameMode.AutoHostOrClient)
                            clientCount = 1;
                        else
                            clientCount = 0;
        ...
        {
            if (TryGetSceneRef(out var sceneRef)) StartCoroutine(StartWithClients(GameMode.Single, sceneRef, 0));
        }
        ...
        {
            if (TryGetSceneRef(out var sceneRef)) StartCoroutine(StartWithClients(GameMode.Server, sceneRef, 0));
        }
        ...
        {
            if (TryGetSceneRef(out var sceneRef)) StartCoroutine(StartWithClients(GameMode.Host, sceneRef, 0));
        }
        ...
        {
            if (TryGetSceneRef(out var sceneRef)) StartCoroutine(StartWithClients(GameMode.Shared, sceneRef, 1));
        }
        ...
        protected IEnumerator StartWithClients(GameMode serverMode, SceneRef sceneRef, int clientCount)
        {
            ...
            if (CurrentStage != Stage.Disconnected) yield break;

            var includesServerStart = serverMode != GameMode.Shared && serverMode != GameMode.Client &&
                                      serverMode != GameMode.AutoHostOrClient;

            if (!includesServerStart && clientCount == 0)
            {
                Debug.LogError(
                    $"{nameof(GameMode)} is set to {serverMode}, and {nameof(clientCount)} is set to zero. Starting no network runners.");
                yield break;
            }
            ...
            if (config.PeerMode != NetworkProjectConfig.PeerModes.Multiple)
            {
                ...
                {
                    Debug.LogWarning(
                        $"Instance mode must be set to {nameof(NetworkProjectConfig.PeerModes.Multiple)} to perform a debug start multiple peers. Restricting client count to {maxClientsAllowed}.");
            ...
            if ((serverMode == GameMode.Shared || serverMode == GameMode.AutoHostOrClient ||
                 serverMode == GameMode.Server || serverMode == GameMode.Host) &&
            ...
            if (includesServerStart)
            {
                ...
                var serverTask = InitializeNetworkRunner(_server, serverMode, NetAddress.Any(ServerPort), sceneRef,
                    runner =>
                    {
#if FUSION_DEV
          ...
          Debug.Log($"Server NetworkRunner '{name}' started.");
                ...
                while (serverTask.IsCompleted == false) yield return new WaitForSeconds(1f);
            ...
            if (includesServerStart && AlwaysShowStats && serverMode != GameMode.Shared)
                FusionStats.Create(runner: _server, screenLayout: FusionStats.DefaultLayouts.Left,
                    objectLayout: FusionStats.DefaultLayouts.Left);
        }
        ...
        {
            if (TryGetSceneRef(out var sceneRef)) AddClient(GameMode.Client, sceneRef);
        }
        ...
        {
            if (TryGetSceneRef(out var sceneRef)) AddClient(GameMode.Shared, sceneRef);
        }
      ...
      var clientTask = InitializeNetworkRunner(client, mode, NetAddress.Any(), sceneRef, (runner) => {
        ...
        Debug.Log($"Client NetworkRunner '{name}' started.");
            ...
            if (AlwaysShowStats && LastCreatedClientIndex == 0)
                FusionStats.Create(runner: client, screenLayout: FusionStats.DefaultLayouts.Right,
                    objectLayout: FusionStats.DefaultLayouts.Right);
        ...
        protected IEnumerator StartClients(int clientCount, GameMode serverMode, SceneRef sceneRef = default)
        {
            ...
            while (clientsStartTask.IsCompleted == false) yield return new WaitForSeconds(1f);

            if (clientsStartTask.IsFaulted) Debug.LogWarning(clientsStartTask.Exception);

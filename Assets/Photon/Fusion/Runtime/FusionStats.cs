...
namespace Fusion
{
  ...
  ///     or be created at runtime using the <see cref="Create" /> methods. If created as the child of a
  ...
  [ScriptHelp(BackColor = ScriptHeaderBackColor.Olive)]
    [ExecuteAlways]
    public partial class FusionStats : Behaviour
    {
      /// <summary>
      ...
      public enum StatCanvasTypes
        ...
        /// <summary>
        ///     Interval (in seconds) between Graph redraws. Higher values (longer intervals) reduce CPU overhead, draw calls and
        ///     garbage collection.
        /// </summary>
        [InlineHelp]
        ...
        /// </summary>
        [Header("Layout")] [InlineHelp] [SerializeField]
        private StatCanvasTypes _canvasType;

        /// <summary>
        ///     Enables text labels for the control buttons.
        /// </summary>
        [InlineHelp] [SerializeField] private bool _showButtonLabels = true;


        /// <summary>
        ///     Height of button region at top of the stats panel. Values less than or equal to 0 hide the buttons, and reduce the
        ///     header size.
        /// </summary>
        [InlineHelp] [SerializeField] [Range(0, 200)]
        ...
        /// </summary>
        [InlineHelp] [DrawIf(nameof(_canvasType), (long)StatCanvasTypes.GameObject, Hide = true)] [Range(0, 20f)]
        public float CanvasScale = 5f;

        /// <summary>
        ///     The distance on the Z axis the canvas will be positioned. Allows moving the canvas in front of or behind the parent
        ///     GameObject.
        /// </summary>
        [InlineHelp] [DrawIf(nameof(_canvasType), (long)StatCanvasTypes.GameObject, Hide = true)] [Range(-10, 10f)]
        public float CanvasDistance;

        /// <summary>
        ///     The Rect which defines the position of the stats canvas on a GameObject. Sizes are normalized percentages.(ranges
        ///     of 0f-1f).
        /// </summary>
        [InlineHelp]
        [SerializeField]
        ...
        private Rect _gameObjectRect = new(0.0f, 0.0f, 0.3f, 1.0f);
        ...
        [NormalizedRect]
        private Rect _overlayRect = new(0.0f, 0.0f, 0.3f, 1.0f);

        /// <summary>
        ///     <see cref="FusionStatsGraph.Layouts" /> value which all child <see cref="FusionStatsGraph" /> components will use
        ///     if their <see cref="FusionStatsGraph.Layouts" /> value is set to Auto.
        /// </summary>
        [Header("Fusion Graphs Layout")] [InlineHelp] [SerializeField]
        private FusionStatsGraph.Layouts _defaultLayout;
        ...
        ///     For uses where perfect camera billboarding is not possible (such as VR), this toggle prevents FusionGraph layouts
        ///     being used where text and graphs overlap.
        ///     Normally leave this unchecked, unless you are experiencing corrupted text rendering.
        /// </summary>
        [InlineHelp] [SerializeField] private bool _noTextOverlap;
        ...
        /// </summary>
        [InlineHelp] [SerializeField] private bool _noGraphShader;

        /// <summary>
        ///     Force graphs layout to use X number of columns.
        /// </summary>
        [InlineHelp] [Range(0, 16)] public int GraphColumnCount = 1;

        /// <summary>
        ///     If <see cref="GraphColumnCount" /> is set to zero, then columns will automatically be added as needed to limit
        ///     graphs to this width or less.
        /// </summary>
        [InlineHelp] [SerializeField] [DrawIf(nameof(GraphColumnCount), 0)] [Range(30, SCREEN_SCALE_W)]
        private int _graphMaxWidth = SCREEN_SCALE_W / 4;
        ...
        /// </summary>
        [Header("Data")] [SerializeField] [InlineHelp] [ReadOnly]
        private NetworkRunner _runner;

        /// <summary>
        ///     Editor-Only. If no <see cref="Object" /> is set, this FusionStats will attempt to connect to the NetworkRunner for
        ///     the current selected GameObject.
        /// </summary>
        [InlineHelp] [SerializeField] public bool RunnerFromSelected;
        ...
        /// </summary>
        [InlineHelp] public bool InitializeAllGraphs;

        /// <summary>
        ///     When <see cref="_runner" /> is null and no <see cref="NetworkRunner" /> exists in the current scene, FusionStats
        ///     will continuously attempt to find and connect to an active <see cref="NetworkRunner" /> which matches these
        ///     indicated modes.
        /// </summary>
        [InlineHelp] [ExpandableEnum(ShowInlineHelp = true)]
        ...
        /// </summary>
        [InlineHelp] [SerializeField] [DrawIf(nameof(_enableObjectStats))] [ExpandableEnum(ShowInlineHelp = true)]
        ...
        /// </summary>
        [InlineHelp] [SerializeField] [ExpandableEnum(ShowInlineHelp = true)]
        public FieldsMask<SimulationConnectionStats> _includedNetStats =
            new(typeof(SimulationConnectionStats).GetDefaults);
        ...
        ///     <see cref="SimulationModes" /> specified by <see cref="ConnectTo" />, and connect to that.
        /// </summary>
        [Header("Life-Cycle")] [InlineHelp] [SerializeField]
        public bool AutoDestroy;

        /// <summary>
        ///     Only one instance with the <see cref="Guid" /> can exist if there is no associated <see cref="NetworkObject" />
        ///     <see cref="_object" />. Will destroy any additional instances on Awake.
        /// </summary>
        [InlineHelp] [SerializeField] public bool EnforceSingle = true;

        /// <summary>
        ///     Identifier used to enforce single instances of <see cref="FusionStats" /> when running in Multi-Peer mode.
        ///     When <see cref="EnforceSingle" /> is enabled, only one instance of <see cref="FusionStats" /> with this GUID will
        ///     be active at any time,
        ///     regardless of the total number of peers running.
        /// </summary>
        [InlineHelp] [DrawIf(nameof(EnforceSingle))] [SerializeField]
        public string Guid;
        ...
        /// </summary>
        [InlineHelp] [SerializeField] public Font ValueFont;

        [SerializeField] [HideInInspector] internal Shader GraphShader;

        /// <summary>
        ///     Shows/hides controls in the inspector for defining element colors.
        /// </summary>
        [InlineHelp] [SerializeField] private bool _modifyColors;

        /// <summary>
        ///     The color used for the telemetry graph data.
        /// </summary>
        [InlineHelp] [SerializeField] [DrawIf(nameof(_modifyColors), Hide = true)]
        private Color _graphColorGood = new(0.1f, 0.5f, 0.1f, 1.0f);

        /// <summary>
        ///     The color used for the telemetry graph data.
        /// </summary>
        [InlineHelp] [SerializeField] [DrawIf(nameof(_modifyColors), Hide = true)]
        private Color _graphColorWarn = new(0.75f, 0.75f, 0.2f, 1.0f);

        /// <summary>
        ///     The color used for the telemetry graph data.
        /// </summary>
        [InlineHelp] [SerializeField] [DrawIf(nameof(_modifyColors), Hide = true)]
        private Color _graphColorBad = new(0.9f, 0.2f, 0.2f, 1.0f);

        /// <summary>
        ///     The color used for the telemetry graph data.
        /// </summary>
        [InlineHelp] [SerializeField] [DrawIf(nameof(_modifyColors), Hide = true)]
        private Color _graphColorFlag = new(0.8f, 0.75f, 0.0f, 1.0f);

        [InlineHelp] [SerializeField] [DrawIf(nameof(_modifyColors), Hide = true)]
        private Color _fontColor = new(1.0f, 1.0f, 1.0f, 1f);

        [InlineHelp] [SerializeField] [DrawIf(nameof(_modifyColors), Hide = true)]
        private Color PanelColor = new(0.3f, 0.3f, 0.3f, 1.0f);

        [InlineHelp] [SerializeField] [DrawIf(nameof(_modifyColors), Hide = true)]
        private Color _simDataBackColor = new(0.1f, 0.08f, 0.08f, 1.0f);

        [InlineHelp] [SerializeField] [DrawIf(nameof(_modifyColors), Hide = true)]
        private Color _netDataBackColor = new(0.15f, 0.14f, 0.09f, 1.0f);

        [InlineHelp] [SerializeField] [DrawIf(nameof(_modifyColors), Hide = true)]
        private Color _objDataBackColor = new(0.0f, 0.2f, 0.4f, 1.0f);
        ...
        /// <summary>
        ///     Returns the set serialized <see cref="NetworkObject" /> for this stat window. If that is null, returns the static
        ///     MonitoredNetworkObject,
        ...
        public NetworkObject Object
        {
            get
            {
                ...
                if (EnableObjectStats) return _runner.FindObject(MonitoredNetworkObjectId);
        ...
        private void Awake()
        {
            ...
            if (Application.isPlaying == false)
            {
                ...
                if (_canvas)
                {
                    ...
                    if (EditorApplication.isCompiling == false) EditorApplication.delayCall += CalculateLayout;
                    _layoutDirty = 2;
                }
            ...
            if (Guid == "") Guid = System.Guid.NewGuid().ToString().Substring(0, 13);
        ...
        private void LateUpdate()
        {
            ...
            if (_activeDirty) ReapplyEnabled();
            ...
            if (runnerIsNull || runner.IsShutdown) return;
            ...
            if (RedrawInterval > 0)
            {
                ...
                if (currentime > _delayDrawUntil)
                {
                    ...
                    while (_delayDrawUntil <= currentime) _delayDrawUntil += RedrawInterval;
                }

                if (currentime != _currentDrawTime) return;
            }

            if (EnableObjectStats) RefreshObjectValues();
        ...
        /// </summary>
        /// <param name="enableObjectStats">
        ///     Optional parameter to enable or disable object stats. If null, the current setting is
        ///     used.
        /// </param>
        /// <param name="objectLayout">
        ///     Optional parameter to set the layout for the object stats. If null, the current setting is
        ///     used.
        /// </param>
        /// <param name="screenLayout">
        ///     Optional parameter to set the layout for the screen stats. If null, the current setting is
        ///     used.
        /// </param>
        public void ResetLayout(bool? enableObjectStats = null, DefaultLayouts? objectLayout = null,
            DefaultLayouts? screenLayout = null)
        {
            // Destroy existing built graphs
            var canv = GetComponentInChildren<Canvas>();
            if (canv) DestroyImmediate(canv.gameObject);
            ...
            if (enableObjectStats.GetValueOrDefault() ||
                (enableObjectStats.GetValueOrDefault(true) && hasNetworkObject))
            {
            ...
            ApplyDefaultLayout(
                objectLayout.GetValueOrDefault(hasNetworkObject ? DefaultLayouts.UpperRight : DefaultLayouts.Full),
        ...
        {
            if (_canvasRT) DestroyImmediate(_canvasRT.gameObject);
            _canvasRT = null;
        }
        ...
        private void Initialize()
        {
            ...
            if (Application.isPlaying)
            {
                ...
                else
                {
                    if (FindFirstObjectByType<EventSystem>() == null)
                    {
                        ...
                        if (Application.isPlaying) DontDestroyOnLoad(eventSystemGO);
                    }
                }
            }

            if (_canvasRT == false) GenerateGraphs();
            ...
            if (_canvasRT)
            {
                ...
                foreach (var g in _foundViews) g.Initialize();
        ...
        private void ReapplyEnabled()
        {
            ...
            for (var i = 0; i < _objGraphs.Length; ++i)
            {
                ...
                var enabled = _enableObjectStats && (((long)1 << i) & _includedObjStats.Mask) != 0;
                if (graph == null)
                {
                    if (enabled)
                    {
                ...
                if (_objGraphs[i] != null) graph.gameObject.SetActive(enabled);
            }

            for (var i = 0; i < _netGraphs.Length; ++i)
            {
                var graph = _netGraphs[i];
                var enabled = (((long)1 << i) & _includedNetStats.Mask) != 0;
                if (graph == null)
                {
                    if (enabled)
                    {
                ...
                if (_netGraphs[i] != null) graph.gameObject.SetActive(enabled);
            }
        }
        ...
        {
            if (EnforceSingle && Guid == "") Guid = System.Guid.NewGuid().ToString().Substring(0, 13);
            _activeDirty = true;
            if (_layoutDirty <= 0)
            {
                _layoutDirty = 2;

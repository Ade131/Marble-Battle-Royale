...
namespace Fusion
{
  ...
  [AddComponentMenu("Fusion/Enable On Single Runner")]
    public class EnableOnSingleRunner : Behaviour
    {
        ...
        private static readonly List<Component> reusableComponentsList = new();

        [StaticField(StaticFieldResetMode.None)]
        private static readonly List<Component> reusableComponentsList2 = new();
...
#pragma warning restore IDE0044 // Add readonly modifier
      ...
      /// <summary>
      ///     Collection of components that will be marked for Multi-Peer mode as objects that should only have one enabled
      ///     instance.
      /// </summary>
      [InlineHelp] public Component[] Components = new Component[0];

      /// <summary>
      ///     Prefix for the GUIDs of <see cref="RunnerVisibilityLink" /> components which are added at runtime.
      /// </summary>
      [HideInInspector] [SerializeField] private string _guid = Guid.NewGuid().ToString().Substring(0, 19);

      /// <summary>
      ///     At runtime startup, this adds a <see cref="RunnerVisibilityLink" /> for each component reference to this
      ///     GameObject.
      /// </summary>
      ...
      /// <summary>
      ///     Finds visual/audio nested components on this GameObject and its children, and adds them to the Components
      ///     collection.
      /// </summary>
        ...
        private static Component[] FindRecognizedComponentsOnGameObject(GameObject go)
        {
            try
            {
                ...
                foreach (var comp in reusableComponentsList)
                {
                    ...
                    if (type.IsRecognizedByRunnerVisibility()) reusableComponentsList2.Add(comp);
                }

...
{
  /// <summary>
  ///     Add this Component to the NetworkRunner Prefab or GameObject. If Interest Management is enabled in
  ///     NetworkProjectConfig ReplicationFeatures,
  ///     gizmos will be shown that indicate active Area Of Interest cells. These gizmos are currently NOT applicable to
  ///     Shared Mode and will only
  ...
  [RequireComponent(typeof(NetworkRunner))]
    ...
    public class RunnerAOIGizmos : SimulationBehaviour
    {
        ...
        public GizmoOptionsEnum GizmoOptions =
            GizmoOptionsEnum.ShowActiveServerZones | GizmoOptionsEnum.ShowPlayerInterest;

        public CustomOptions Customization = new()
        {
        ...
        private void OnDrawGizmos()
        {
            ...
            if (GizmoOptions == 0) return;

            var runner = Runner;

            if ((object)runner == null || runner.IsRunning == false) return;

            var datas = _reusableGizmoData ??=
                new List<(Vector3 center, Vector3 size, int playerCount, int objectCount)>();

...
namespace Fusion
{
    ...
    public class RunnerLagCompensationGizmos : Behaviour
    {
        ...
        {
            if (_runner == null || _runner.IsRunning == false || _runner.GetVisible() == false ||
                _runner.LagCompensation.DrawInfo == default) return;

            if (DrawBroadphaseNodes) RenderBHVBroadphase();

            if (DrawSnapshotHistory) RenderHitboxHistory();
        }
            ...
            foreach (var colliderDrawInfo in snapshotDrawInfo)
            {
                ...
                switch (colliderDrawInfo.Type)
                {
                    ...
                    case HitboxTypes.Capsule:
                        LagCompensationDraw.GizmosDrawWireCapsule(colliderDrawInfo.CapsuleTopCenter,
                            colliderDrawInfo.CapsuleBottomCenter, colliderDrawInfo.Radius);
                        break;
                    default:

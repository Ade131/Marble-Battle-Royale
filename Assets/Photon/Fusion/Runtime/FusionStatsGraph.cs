  ...
  public class FusionStatsGraph : FusionStatsGraphBase
    {
        ...
        private const int MAX_FONT_SIZE_MIN_MAX = 32;
        private const int MAX_TITLE_SIZE = 26;
        private const int MAX_AVG_SIZE = 200;

        private const int GRPH_TOP_PAD = 36;
        private const int GRPH_BTM_PAD = 36;
        private const int HIDE_XTRAS_WDTH = 200;

        private const int EXPAND_GRPH_THRESH = GRPH_BTM_PAD + GRPH_TOP_PAD + 40;
        private const int COMPACT_THRESH = GRPH_BTM_PAD + GRPH_TOP_PAD - 10;

        private static float[] reusableTimes = new float[OverTimeStatBuffer.CAPACITY];

        // Cached strings for UI/Shader
        private static readonly string[] _decimalFormats = { "F0", "F1", "F2", "F3", "F4", "F5", "F6" };
        ...
        /// <summary>
        ///     Select between automatic formatting (based on size and aspect ratio of the graph) and manual selection of the
        ///     various graph layouts available.
        /// </summary>
        [InlineHelp] [SerializeField] [Header("Graph Layout")]
        private Layouts _layout;

        /// <summary>
        ///     Controls if the graph shader will render. Currently the graph shader only works in Overlay mode, so if forced to
        ///     show Always here, it will not render correctly in 3d space.
        /// </summary>
        [InlineHelp] [SerializeField] private ShowGraphOptions _showGraph = ShowGraphOptions.Always;

        /// <summary>
        ///     Padding added to text and other layout objects.
        /// </summary>
        [InlineHelp] public float Padding = 5f;

        /// <summary>
        ///     The graph shaded area (which is only visible in Overlay mode) will expand to the full extends of the
        ///     <see cref="FusionStatsGraph" /> component when this is enabled, regardless of size.
        ///     When false, the graph will automatically expand only as needed.
        /// </summary>
        [InlineHelp] [SerializeField] private bool _alwaysExpandGraph;
        ...
        private float[] _intensity = new float[OverTimeStatBuffer.CAPACITY];
        private float _max;

        private float _min;
        private float[] _values = new float[OverTimeStatBuffer.CAPACITY];
...
#if UNITY_EDITOR
            if (Selection.activeGameObject == gameObject) EditorApplication.delayCall += CalculateLayout;
#endif
            _layoutDirty = _layoutDirty < 1 ? 1 : _layoutDirty;
        }
#endif
        ...
        public void Clear()
        {
            if (_values != null && _values.Length > 0)
            {
                ...
                for (var i = 0; i < _intensity.Length; ++i) _intensity[i] = -2;
                _min = 0;
                _max = 0;
            }
        }
        ...
        private void ResetGraphShader()
        {
            if (GraphImg)
            {
                var desiredShader = FusionStats != null
                    ? FusionStats.CanvasType == FusionStats.StatCanvasTypes.GameObject
                        ? ShaderType.GameObject
                        : ShaderType.Overlay
                    : ShaderType.None;
                ...
                var shader = _fusionStats.GraphShader; // Resources.Load<Shader>("FusionStatsGraphShader");
        ...
        public override void Refresh()
        {
            ...
            if (_values == null)
            {
                var size = OverTimeStatBuffer.CAPACITY;
        ...
        private void UpdateGraph()
        {
            ...
            var peak = 0f;
            var avgCount = 0;
            var last = values.Length > 0 ? values[^1] : 0;

            for (var i = 0; i < statsBuffer.Length; ++i)
            {
                var v = values[i] * multiplier;
                ...
                var intensity =
                    val < 0
                        ...
                        val >= ErrorThreshold
                            ...
                            val >= WarnThreshold
                                ?
                                Mathf.Lerp(.5f, 1f, (val - WarnThreshold) / (ErrorThreshold - WarnThreshold))
                                :
                                0f;
        ...
        /// </summary>
        public static FusionStatsGraph Create(FusionStats fusionStats, StatSourceTypes statSourceType, int statId,
        ...
        public void Generate(StatSourceTypes type, int statId, RectTransform root)
        {
            ...
            BackImage = background.gameObject.AddComponent<Image>();
            ...
            GraphImg = graphRT.gameObject.AddComponent<Image>();
            ...
            _avgBttn = avgRT.gameObject.AddComponent<Button>();

            // Main Center value
            var perRT = root.CreateRectTransform("Per")
                .SetAnchors(0.3f, 0.7f, 0.0f, 0.125f)
                .SetOffsets(MRGN, -MRGN, MRGN, 0.0f);
            LabelPer = perRT.AddText("avg per XXX", TextAnchor.LowerCenter, fontColor, labelFont,
                MAX_FONT_SIZE_MIN_MAX);
        ...
        public override void CalculateLayout()
        {
            ...
            if (StatSourceInfo == null) TryConnect();
            ApplyTitleText();
            ...
            GraphImg.material.SetInt("_ZWrite",
                fusionStats.CanvasType == FusionStats.StatCanvasTypes.GameObject ? 1 : 0);

            var isOverlayCanvas = fusionStats.CanvasType == FusionStats.StatCanvasTypes.GameObject;
            var vrSafe =
                fusionStats.NoGraphShader /*&& _fusionStats.CanvasType == FusionStats.StatCanvasTypes.GameObject*/;
            ...
            if (fusionStats.NoTextOverlap)
            {
                ...
                else if (layout == Layouts.FullAuto) layout = Layouts.FullNoOverlap;
            }

            var noGraph = vrSafe || layout == Layouts.CompactNoGraph || layout == Layouts.CenteredNoGraph ||
                          (fusionStats.NoTextOverlap && layout == Layouts.CompactAuto);
            var noOverlap = fusionStats.NoTextOverlap || layout == Layouts.FullNoOverlap ||
                            layout == Layouts.CenteredNoOverlap;
            var showGraph = !noGraph && (ShowGraph == ShowGraphOptions.Always ||
                                         (ShowGraph == ShowGraphOptions.OverlayOnly && isOverlayCanvas));

            var expandGraph = !noOverlap && (_alwaysExpandGraph || !showGraph || layout == Layouts.CompactAuto ||
                                             (!noOverlap && height < EXPAND_GRPH_THRESH));
            ...
            var showExtras =
                layout == Layouts.FullAuto ||
                layout == Layouts.FullNoOverlap /*|| (layout == Layouts.Compact && width > HIDE_XTRAS_WDTH)*/;

...
{
  /// <summary>
  ///     Companion component for <see cref="FusionBootstrap" />. Automatically added as needed for rendering in-game
  ///     networking IMGUI.
  /// </summary>
  [RequireComponent(typeof(FusionBootstrap))]
    ...
    {
      /// <summary>
      ///     When enabled, the in-game user interface buttons can be activated with the keys H (Host), S (Server) and C
      ///     (Client).
      /// </summary>
      [InlineHelp] public bool EnableHotkeys;
      ...
      [InlineHelp] public GUISkin BaseSkin;
        ...
        private void Update()
        {
            ...
            if (nds.StartMode != FusionBootstrap.StartModes.UserInterface) return;

            var currentstage = nds.CurrentStage;
            if (currentstage != FusionBootstrap.Stage.Disconnected) return;

            if (EnableHotkeys)
            {
                if (Input.GetKeyDown(KeyCode.I)) _networkDebugStart.StartSinglePlayer();
        ...
        protected virtual void OnGUI()
        {
            ...
            var currentstage = nds.CurrentStage;
            if (nds.AutoHideGUI && currentstage == FusionBootstrap.Stage.AllConnected) return;
                ...
                GUILayout.BeginVertical(GUI.skin.window);
                {
                    if (currentstage == FusionBootstrap.Stage.Disconnected)
                    {
                        GUILayout.BeginHorizontal();
                        {
                            ...
                            nds.DefaultRoomName =
                                GUILayout.TextField(nds.DefaultRoomName, 25, GUILayout.Height(height));
                        }
                        GUILayout.EndHorizontal();

                        if (GUILayout.Button(EnableHotkeys ? "Start Single Player (I)" : "Start Single Player",
                                GUILayout.Height(height))) nds.StartSinglePlayer();

                        if (GUILayout.Button(EnableHotkeys ? "Start Shared Client (P)" : "Start Shared Client",
                        ...
                        if (GUILayout.Button(EnableHotkeys ? "Start Server (S)" : "Start Server",
                                GUILayout.Height(height)))
                        {
                            if (_isMultiplePeerMode)
                        ...
                        if (GUILayout.Button(EnableHotkeys ? "Start Client (C)" : "Start Client",
                                GUILayout.Height(height)))
                        {
                            if (_isMultiplePeerMode)
                                StartMultipleClients(nds);
                        ...
                        if (GUILayout.Button(
                                EnableHotkeys ? "Start Auto Host Or Client (A)" : "Start Auto Host Or Client",
                                GUILayout.Height(height)))
                        {
                            if (_isMultiplePeerMode)
                                StartMultipleAutoClients(nds);
                        ...
                        if (_isMultiplePeerMode)
                        {
                            GUILayout.BeginHorizontal( /*GUI.skin.button*/);
                            {
                                ...
                                var newcount = GUILayout.TextField(_clientCount, 10, GUILayout.Width(width * .25f),
                                    GUILayout.Height(height));
                    ...
                    {
                        if (GUILayout.Button("Shutdown", GUILayout.Height(height))) _networkDebugStart.ShutdownAll();
                    }
            ...
            Dictionary<T, string> nonalloc = null) where T : Enum
        {
            ...
            for (int i = 0, cnt = names.Length; i < cnt; ++i)
            {
                ...
                for (var n = 0; n < name.Length; n++)
                {
                    ...
                    if (char.IsUpper(name[n]) && n != 0) sb.Append(" ");

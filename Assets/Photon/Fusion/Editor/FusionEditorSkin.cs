...
namespace Fusion.Editor
{
  ...
  [CreateAssetMenu(menuName = "Fusion/Editor Skin")]
    ...
    internal class FusionEditorSkin : ScriptableObject
    {
        private static FusionEditorSkin s_Instance;

        public static readonly LazyAsset<Texture2D> InfoIcon = LazyAsset.Create(() =>
            FindTextureOrThrow(EditorGUIUtility.isProSkin ? "d_console.infoicon.sml@2x" : "console.infoicon.sml@2x"));

        public static readonly LazyAsset<Texture2D> WarningIcon = LazyAsset.Create(() =>
            FindTextureOrThrow(EditorGUIUtility.isProSkin ? "d_console.warnicon.sml@2x" : "console.warnicon.sml@2x"));

        public static readonly LazyAsset<Texture2D> ErrorIcon = LazyAsset.Create(() =>
            FindTextureOrThrow(EditorGUIUtility.isProSkin ? "d_console.erroricon.sml@2x" : "console.erroricon.sml@2x"));

        public static readonly LazyAsset<Texture2D> LoadStateIcon =
            LazyAsset.Create(() => FindTextureOrThrow("blendSampler"));

        public static readonly LazyAsset<Texture2D> RefreshIcon = LazyAsset.Create(() =>
            FindTextureOrThrow(EditorGUIUtility.isProSkin ? "d_Refresh@2x" : "Refresh@2x"));
        ...
        public static readonly LazyGUIStyle OverlayLabelStyle = LazyGUIStyle.Create(_ =>
        {
            ...
            result.normal.textColor = EditorGUIUtility.isProSkin
                ? new Color(255f / 255f, 221 / 255f, 0 / 255f, 1f)
                : Color.blue;
            return result;
        });

        public static readonly LazyGUIStyle RawDataStyle =
            LazyGUIStyle.Create(_ => new GUIStyle(EditorStyles.textArea) { wordWrap = true });

        public GUISkin Skin;

        private readonly Dictionary<ScriptHeaderBackColor, Color> _scriptHeaderStyles = new()
        {
        ...
        public static GUIStyle HelpButtonStyle =>
            instance.Skin.GetStyle(EditorGUIUtility.isProSkin ? "dark-help-button" : "light-help-button");
        ...
        public static GUIStyle RichLabelStyle =>
            instance.Skin.GetStyle(EditorGUIUtility.isProSkin ? "dark-rich-label" : "light-rich-label");
        ...
        public static Color HelpInlineBoxColor => EditorGUIUtility.isProSkin
            ? new Color(0.317f, 0.337f, 0.352f, 1.000f)
            : new Color(0.686f, 0.776f, 0.859f);

        public static Color WarningInlineBoxColor => EditorGUIUtility.isProSkin
            ? new Color(0.36f, 0.33f, 0.22f, 1.00f)
            : new Color(0.98f, 0.94f, 0.80f, 0.90f);

        public static Color ErrorInlineBoxColor => EditorGUIUtility.isProSkin
            ? new Color(0.40f, 0.15f, 0.10f, 1.00f)
            : new Color(0.9f, 0.70f, 0.70f, 1.00f);

        private static Texture2D FindTextureOrThrow(string id)
        {
            return EditorGUIUtility.FindTexture(id) ??
        ...
        {
            if (instance._scriptHeaderStyles.TryGetValue(settingsBackColor, out var color)) return color;
            return default;
        }
    }
}
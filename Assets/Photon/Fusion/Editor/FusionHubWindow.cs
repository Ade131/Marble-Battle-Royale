...
namespace Fusion.Editor
{
    ...
    public partial class FusionHubWindow : EditorWindow
    {
        ...
        private static Vector2 _windowPosition = new(100, 100);
        ...
        private void OnGUI()
        {
            ...
            _windowPosition = position.position;
        ...
        {
            if (_ready.HasValue && _ready.Value == false) Open();
        ...
        private void DrawSetupSection()
        {
            ...
            DrawButtonAction(Icon.PhotonCloud, "Photon App Settings",
                "Network settings specific to the Photon transport.",
                callback: () =>
                {
        ...
        {
            DrawButtonAction(Icon.Documentation, "Fusion Introduction", "The Fusion Introduction web page.",
                callback: OpenURL(Constants.UrlFusionIntro));
            DrawButtonAction(Icon.Documentation, "SDK and Release Notes", "Link to the latest Fusion version SDK.",
                callback: OpenURL(Constants.UrlFusionSDK));
            DrawButtonAction(Icon.Documentation, "API Reference", "The API library reference documentation.",
                callback: OpenURL(Constants.UrlFusionDocApi));
        }

        private void DrawSamplesSection()
        {
            GUILayout.Label("Tutorials", headerLabelStyle);
            DrawButtonAction(Icon.Samples, "Fusion 100 Tutorial", "Fusion Fundamentals Tutorial",
                callback: OpenURL(Constants.UrlFusion100));
        }
        ...
        private void DrawReleaseHistoryItem(string label, List<string> items)
        {
            if (items != null && items.Count > 0)
            {
                GUILayout.BeginVertical();
                {
                    ...
                    foreach (var text in items) GUILayout.Label(string.Format("- {0}.", text), textLabelStyle);
                }
        ...
        private void DrawSupportSection()
        {
            ...
            DrawButtonAction(Icon.Community, Constants.DiscordHeader, Constants.DiscordText,
                callback: OpenURL(Constants.UrlDashboardProfile));
            DrawButtonAction(Icon.Documentation, Constants.DocumentationHeader, Constants.DocumentationText,
                callback: OpenURL(Constants.UrlFusionDocsOnline));
        }

        private void DrawSetupAppIdBox()
        {
            var realtimeSettings = PhotonAppSettings.Global;
            ...
            using (new EditorGUILayout.VerticalScope(FusionHubSkin.GetStyle("SteelBox") /*contentBoxStyle*/))
            {
                ...
                DrawButtonAction(Icon.PhotonCloud, "Open the Photon Dashboard",
                    callback: OpenURL(Constants.UrlDashboard));
                ...
                using (new EditorGUILayout.HorizontalScope(FusionHubSkin.GetStyle("SteelBox")))
                {
                    ...
                    var editedAppId = EditorGUILayout.TextField("", realtimeAppId, FusionHubSkin.textField,
                        GUILayout.Height(24));
        ...
        private void DrawLeftNavMenu()
        {
            for (var i = 0; i < _sections.Length; ++i)
            {
                ...
                if (DrawNavButton(section, _currentSection == i)) _currentSection = i;
            }
        }
        ...
        private void DrawButtonAction(Icon icon, string header, string description = null, bool? active = null,
            Action callback = null, int? width = null)
        {
            DrawButtonAction(GetIcon(icon), header, description, active, callback, width);
        }

        private static void DrawButtonAction(Texture2D icon, string header, string description = null,
            bool? active = null, Action callback = null, int? width = null)
        {
            var padding = GUI.skin.button.padding.top + GUI.skin.button.padding.bottom;
            var height = icon.height + padding;

            var renderStyle = active.HasValue && active.Value ? buttonActiveStyle : GUI.skin.button;
            // Draw text separately (not part of button guiconent) to have control over the space between the icon and the text.
            var rect = EditorGUILayout.GetControlRect(false, height,
                width.HasValue ? GUILayout.Width(width.Value) : GUILayout.ExpandWidth(true));
            ...
            if (clicked && callback != null) callback.Invoke();
        }
        ...
        private class FusionHubPostProcessor : AssetPostprocessor
        {
            ...
            /// <summary>
            ///     Attempts enforce existence of singleton. If Editor is not ready, this method will be deferred one editor update and

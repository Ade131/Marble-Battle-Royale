...
namespace Fusion.Editor
{
  ...
  ///     UI toggles for runner SetVisible() and ProvideInput members. NetworkRunner and Player Objects can be pinged in the
  ///     hierarchy.
  ...
  public class FusionRunnerVisibilityControlsWindow : EditorWindow
    {
        ...
        private static readonly Lazy<GUIStyle> s_labelStyle = new(() => new GUIStyle(EditorStyles.label)
            { fontSize = FONT_SIZE });

        private static readonly Lazy<GUIStyle> s_labelTinyStyle = new(() => new GUIStyle(s_labelStyle.Value)
            { fontSize = FONT_SIZE - 1 });

        private static Lazy<GUIStyle> s_labelCenterStyle = new(() => new GUIStyle(s_labelStyle.Value)
            { alignment = TextAnchor.MiddleCenter });

        private static readonly Lazy<GUIStyle> s_buttonStyle = new(() => new GUIStyle(EditorStyles.miniButton)
            { fontSize = FONT_SIZE });

        private static Lazy<GUIStyle> s_helpboxStyle = new(() => new GUIStyle(EditorStyles.helpBox)
        {
            fontSize = FONT_SIZE, alignment = TextAnchor.MiddleCenter, padding = new RectOffset(6, 6, 6, 6)
        });

        private static readonly Lazy<GUIStyle> s_invisibleButtonStyle = new(() => new GUIStyle(EditorStyles.label)
            { fontSize = FONT_SIZE, padding = new RectOffset() });

        private static readonly Lazy<GUIStyle> s_invisibleButtonGrayStyle = new(() => new GUIStyle(EditorStyles.label)
        {
            fontSize = FONT_SIZE,
        ...
        private static readonly Lazy<string> Dark = new(() => EditorGUIUtility.isProSkin ? "d_" : "");

        private static readonly Lazy<GUIContent> s_runnerGC =
            new(() => new GUIContent(string.Empty, Labels.RunnerTooltip));

        private static readonly Lazy<GUIContent> s_playerObjGC =
            new(() => new GUIContent(string.Empty, Labels.PlayerObjTooltip));

        private static readonly Lazy<GUIContent> s_visibleIcon = new(() =>
            new GUIContent(EditorGUIUtility.FindTexture(Dark.Value + "scenevis_visible_hover@2x"),
                Labels.VisibilityTooltip));

        private static readonly Lazy<GUIContent> s_hiddenIcon = new(() =>
            new GUIContent(EditorGUIUtility.FindTexture(Dark.Value + "scenevis_hidden@2x"), Labels.VisibilityTooltip));

        private static readonly Lazy<GUIContent> s_inputIconLong =
            new(() => new GUIContent(Labels.ProvidingInputs,
                EditorGUIUtility.FindTexture(Dark.Value + "UnityEditor.GameView@2x"), Labels.InputTooltip));

        private static readonly Lazy<GUIContent> s_inputIconShort = new(() => new GUIContent(null,
            EditorGUIUtility.FindTexture(Dark.Value + "UnityEditor.GameView@2x"), Labels.InputTooltip));

        private static readonly Lazy<GUIContent> s_noInputIconLong = new(() => new GUIContent(Labels.NoInputs,
            EditorGUIUtility.FindTexture(Dark.Value + "Toolbar Minus@2x"), Labels.InputTooltip));

        private static readonly Lazy<GUIContent> s_noInputIconShort = new(() =>
            new GUIContent(null, EditorGUIUtility.FindTexture(Dark.Value + "Toolbar Minus@2x"), Labels.InputTooltip));

        private static readonly Lazy<GUIContent> s_noVisibilityWarn =
            new(() => new GUIContent(FusionEditorSkin.WarningIcon, Labels.NoVisibilityWarn));

        private static readonly Lazy<GUIContent> s_statsGC =
            new(() => new GUIContent(string.Empty, Labels.StatsTooltip));

        private readonly Dictionary<NetworkRunner, FusionStats> _stats = new();
        ...
        private void Update()
        {
            ...
            if (Time.realtimeSinceStartup - _lastRepaintTime > REFRESH_RATE) Repaint();
        }
        ...
        private void OnGUI()
        {
            ...
            else
            {
                ...
                while (enumerator.MoveNext())
                {
                    ...
                    DrawRunnerRow(runner, shortText, isWide, currentViewWidth,
                        ref runnerWithoutVisibilityEnabledDetected);
                }
        ...
        private void DrawRunnerRow(NetworkRunner runner, bool shortText, bool isWide, float currentViewWidth,
            ref bool runnerWithoutVisibilityEnabledDetected)
        {
            ...
            if (Application.isPlaying && (!runner || !runner.IsRunning)) return;
            ...
            using (new EditorGUI.DisabledGroupScope(runnerIsNull))
            {
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    var runnerName = runnerIsNull ? Labels.NoActiveRunner :
                        shortText ? runner.IsServer
                            ? runner.IsSinglePlayer ? Labels.SP : runner.IsPlayer ? Labels.H : Labels.S
                            : Labels.C :
                        runner.name;
                    ...
                    runnerGC.text = runnerIsNull ? Labels.NoRunners :
                        runner.IsSharedModeMasterClient ? $"{runnerName} [MC]" : runnerName;
                    var runnerRect = EditorGUILayout.GetControlRect(GUILayout.ExpandWidth(true),
                        GUILayout.MinWidth(isWide ? RUNNR_BTTN_WIDE : RUNNR_BTTN_SLIM));
                    ...
                    using (new EditorGUI.DisabledGroupScope(isSinglePeer))
                    {
                        ...
                        if (isSinglePeer || runner.HasVisibilityEnabled())
                        {
                            if (GUI.Button(toggleRect,
                                    isSinglePeer || runner.GetVisible() ? s_visibleIcon.Value : s_hiddenIcon.Value,
                                    s_invisibleButtonStyle.Value))
                            {
                                if ((Event.current.modifiers & (EventModifiers.Shift | EventModifiers.Control |
                                                                EventModifiers.Command | EventModifiers.Alt)) == 0)
                                {
                                ...
                                else
                                {
                                    ...
                                    while (others.MoveNext())
                                    {
                                        ...
                                        if (!other || !other.IsRunning) continue;
                    ...
                    using (new EditorGUI.DisabledGroupScope(runnerIsNull || runner.Mode == SimulationModes.Server))
                    {
                        ...
                        var inputContent = isWide ? providingInput ? s_inputIconLong.Value : s_noInputIconLong.Value :
                            providingInput ? s_inputIconShort.Value : s_noInputIconShort.Value;

                        if (GUI.Button(inputToggleRect, inputContent,
                                providingInput ? s_invisibleButtonStyle.Value : s_invisibleButtonGrayStyle.Value))
                        {
                            if ((Event.current.modifiers & (EventModifiers.Shift | EventModifiers.Control |
                                                            EventModifiers.Command | EventModifiers.Alt)) == 0)
                            {
                    ...
                    {
                        var statsLeftRect =
                            EditorGUILayout.GetControlRect(GUILayout.Width(isWide ? STATS_BTTN_WIDE : STATS_BTTN_SLIM));
                        var statsRightRect =
                            EditorGUILayout.GetControlRect(GUILayout.Width(isWide ? STATS_BTTN_WIDE : STATS_BTTN_SLIM));
                    ...
                    if (currentViewWidth > 600)
                        using (new EditorGUI.DisabledGroupScope(true))
                        {
                            ...
                            var userIdRect =
                                EditorGUILayout.GetControlRect(GUILayout.MinWidth(40), GUILayout.ExpandWidth(true));
                            GUI.Label(userIdRect, Labels.UserID + (userIdString ?? Labels.Dash),
                                s_labelTinyStyle.Value);
                        }
                }
        ...
        private static class Labels
        {
            ...
            public const string StatsTooltip =
                "Clicking this button at runtime will create a FusionStats overlay associated with this NetworkRunner. ";

            public const string RunnerTooltip =
                "The name of the NetworkRunner this row controls. Clicking this button will ping the NetworkRunner GameObject in the hierarchy.";
            ...
            public static readonly string RunnerVisibilityNotEnabled =
                $"Runner visibility not enabled. Add a {nameof(RunnerEnableVisibility)} component to your {nameof(NetworkRunner)} Prefab.";
        }
    }
}
...
namespace Fusion.Menu
{
  ...
  [ScriptHelp(BackColor = ScriptHeaderBackColor.Blue)]
    ...
    public class FusionMenuConfig : FusionScriptableObject, IFusionMenuConfig
    {
      ...
      ///     PhotonMeneSceneInfo.Preview = a sprite with a preview of the scene (screenshot) that is displayed in the main menu
      ///     and scene selection screen (can be null)
      /// </summary>
      [InlineHelp] [SerializeField] protected List<PhotonMenuSceneInfo> _availableScenes;

      /// <summary>
      ///     The <see cref="FusionMenuMachineId" /> ScriptableObject that stores local ids to use as an option in for
      ///     AppVersion.

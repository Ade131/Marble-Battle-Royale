...
namespace Fusion.Menu
{
    ...
    using InputField = TMP_InputField;
    using Text = TMP_Text;
#else
      ...
      public override void Awake()
        {
            ...
            {
                if (int.TryParse(s, out var maxPlayers) == false || maxPlayers <= 0 ||
      ...
      _goFullscreenn.SetActive(false);
        ...
        public override void Show()
        {
            ...
            _entryAppVersion.SetOptions(_appVersions, ConnectionArgs.AppVersion,
                s => s.Equals(Config.MachineId) ? $"Build ({Config.MachineId})" : s);
            _entryFramerate.SetOptions(_graphicsSettings.CreateFramerateOptions, _graphicsSettings.Framerate,
                s => s == -1 ? "Platform Default" : s.ToString());
        ...
        Screen.resolutions[s].ToString());
            ...
            _entryGraphicsQuality.SetOptions(_graphicsSettings.CreateGraphicsQualityOptions,
                _graphicsSettings.QualityLevel, s => QualitySettings.names[s]);
            _uiMaxPlayers.SetTextWithoutNotify(Math.Clamp(ConnectionArgs.MaxPlayerCount, 1, Config.MaxPlayerCount)
                .ToString());
        ...
        protected virtual void SaveChanges()
        {
            ...
            if (int.TryParse(_uiMaxPlayers.text, out var maxPlayers))
            {

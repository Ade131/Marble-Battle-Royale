...
namespace Fusion.Addons.Physics
{
    public abstract partial class NetworkRigidbody<RBType, PhysicsSimType>
    {
        ...
        public override void Render()
        {
            ...
            if (_doNotInterpolate) return;

            var isInSimulation = Object.IsInSimulation;
            var physicsSimulatorExists = (object)_physicsSimulator != null;

            // If Unity is auto-simulating, only non-simulated proxies should be interpolated
            if (isInSimulation && (physicsSimulatorExists == false ||
                                   _physicsSimulator.PhysicsAuthority == PhysicsAuthorities.Unity)) return;
            ...
            if (Object.RenderSource == RenderSource.Latest) return;
            ...
            if (TryGetSnapshotsBuffers(out var fr, out var to, out var alpha))
            {
                ...
                if (teleport && toKey >= 0) toData = frData;
                ...
                if (syncParent)
                {
                    ...
                    {
                        var frHasNonNetworkedParent = frData.TRSPData.Parent == NetworkTRSPData.NonNetworkedParent;
                        ...
                        {
                            Debug.LogError(
                                $"Parent of this object is not present {frData.TRSPData.Parent} {frData.TRSPData.Parent.Behaviour}.");
                            return;
                        }
                        ...
                        if (frHasNonNetworkedParent || frData.TRSPData.Parent != toData.TRSPData.Parent)
                        {
                            ...
                            if (syncScale) tr.localScale = frData.TRSPData.Scale;
                            _rootIsDirtyFromInterpolation = true;
                            return;
                        }
                    ...
                    else
                    {
                        ...
                        if (frData.TRSPData.Parent != toData.TRSPData.Parent)
                        {
                            if (useTarget)
                            {
                                ...
                                if (syncScale)
                                    Debug.LogWarning(
                                        $"{GetType().Name} cannot sync scale when using an interpolation target.");
#endif
                ...
                else
                {
                    ...
                    rot = Quaternion.Slerp(frRotation,
                        UsePreciseRotation ? toData.FullPrecisionRotation : toData.TRSPData.Rotation, alpha);
                }
                ...
                else
                {
                    ...
                    if (!hasInterpolationTarget && !_targIsDirtyFromInterpolation && UseRenderSleepThresholds)
                    {
                        ...
                        if (
                            ...
                            (thresholds.Position == 0 || (pos - tr.position).sqrMagnitude <
                                thresholds.Position * thresholds.Position) &&
                            (thresholds.Rotation == 0 || Quaternion.Angle(rot, tr.rotation) < thresholds.Rotation) &&
                            (thresholds.Scale == 0 || !syncScale || (scl - tr.localScale).sqrMagnitude <
                    ...
                    if (syncScale) tr.localScale = scl;
                    _rootIsDirtyFromInterpolation = true;
                }
            }
            else

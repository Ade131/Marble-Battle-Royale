  ...
  public abstract partial class NetworkRigidbody<RBType, PhysicsSimType> : IBeforeAllTicks, IAfterTick
    {
        ...
        void IAfterTick.AfterTick()
        {
            ...
            var remainingTicks = _remainingSimulationsCount--;
            if (remainingTicks > 2) return;
            ...
            else
            {
                ...
                if (Object.IsInSimulation) CopyToBuffer(true);
            }
        }
        ...
        void IBeforeAllTicks.BeforeAllTicks(bool resimulation, int tickCount)
        {
            ...
            if (_targIsDirtyFromInterpolation && _interpolationTarget)
            {
                ...
                if (SyncScale) _interpolationTarget.localScale = new Vector3(1f, 1f, 1f);
            }
            ...
            if (_rootIsDirtyFromInterpolation || (_clientPrediction && resimulation)) CopyToEngine(resimulation);
        }
        ...
        /// </summary>
        /// <param name="captureTRSPOnly">
        ///     If the captured data is only used for interpolation (not for networking), then skip
        ///     capturing extras (vel/angVel/tensor)
        /// </param>
        protected virtual void CopyToBuffer(bool captureTRSPOnly)
        {
            ...
            if (syncParent)
            {
                ...
                if (IsMainTRSP)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (_aoiEnabled) SetAreaOfInterestOverride(parentNB.Object);
            ...
            if (SyncScale) Data.TRSPData.Scale = tr.localScale;
        ...
        /// </summary>
        /// <param name="predictionReset">
        ///     Indicates if this is a reset from a remote server state, in which case everything needs to be reverted to state
        ///     Otherwise, if it is for the State Authority or a non-simulated proxy - only the TRSP needs to be reset from
        ///     interpolation changes. (not velocity etc)
        /// </param>
        protected virtual void CopyToEngine(bool predictionReset)
        {
            ...
            if (syncParent)
            {
                ...
                var hasNonNetworkedParent = Data.TRSPData.Parent == NetworkTRSPData.NonNetworkedParent;
                ...
                else
                {
                    ...
                    if (currentParent) tr.SetParent(null);
                }
            }
            else
            {
            ...
            var avoidWaking = !rootIsDirty && currentIsSleeping && networkedIsSleeping && tr.localPosition == pos &&
            ...
            if (avoidWaking == false)
            {
                ...
                if (isParented == false || useWorldSpace) ApplyRBPositionRotation(rb, pos, rot);
                _rootIsDirtyFromInterpolation = false;
            }

            if (SyncScale) tr.localScale = Data.TRSPData.Scale;

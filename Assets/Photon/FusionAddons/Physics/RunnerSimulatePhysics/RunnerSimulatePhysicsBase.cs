...
namespace Fusion.Addons.Physics
{
  ...
  public abstract class RunnerSimulatePhysicsBase : SimulationBehaviour, IBeforeTick
    {
      /// <summary>
      ///     Stored original Physics setting auto-simulate setting, used to restore Unity settings when Fusion runners are
      ///     shutdown.
      /// </summary>
      ...
      /// </summary>
      [InlineHelp] [SerializeField] protected PhysicsAuthorities _physicsAuthority = PhysicsAuthorities.Fusion;
      ...
      /// <summary>
      ///     If enabled, clients will only call Physics.Simulate() on forward Ticks (a Tick which is being simulated for the
      ///     first time),
      ///     and will not simulate physics for re-sims. In nearly all use cases this should be set to false.
      ///     Only enable this if you know that NO physics prediction exists in your game (all Rigidbody movement is client
      ///     authority).
      ///     Not applicable to Shared Mode, as there are no re-simulation nor prediction in that mode and all simulations are
      ///     forward.
      /// </summary>
      [InlineHelp] [DrawIf(nameof(ShowForwardOnly), Hide = true)] [SerializeField]
        public bool ForwardOnly;

      /// <summary>
      ///     <para>
      ///         This value is used to scale PhysicsSimulationDeltaTime, typically to speed up and slow down the passing of
      ///         time.
      ///         This doesn't change the Fusion TickRate (that value is fixed and cannot be changed once a game is started),
      ///         and instead changes how much time is simulated each Physics tick. When changing this value, be sure to account
      ///         for it
      ///         in all code where you use <see cref="NetworkRunner.DeltaTime" />, as you likely ill want to apply the same
      ///         modifier everywhere.
      ///     </para>
      ///     <para>
      ///         For Physics.Simulate(deltaTime) - the deltaTime is calculated as PhysicsSimulationDeltaTime *
      ///         DeltaTimeMultiplier.
      ...
      /// </summary>
      [InlineHelp] [SerializeField] public bool SetUnityFixedTimestep;
        ...
        ///     Used by Fusion inspector UI.
        /// </summary>
        internal bool WarnAutoSyncTransforms => AutoSyncTransforms && _physicsAuthority != PhysicsAuthorities.Unity &&
                                                _physicsTiming != PhysicsTimings.Update;
        ...
        ///         Override this if you want to control how much time passes in each tick (for bullet-time or time compression
        ///         effects).
        ...
        ///     </para>
        /// </summary>
        public virtual float PhysicsSimulationDeltaTime => Runner.DeltaTime;
        ...
        public void FixedUpdate()
        {
            ...
            if (_physicsTiming == PhysicsTimings.FixedUpdate)
            {
                ...
                if (SetUnityFixedTimestep) Time.fixedDeltaTime = Runner.DeltaTime;
            }
            else
            {
        ...
        private void OnValidate()
        {
            ...
            {
                Debug.LogWarning(
                    $"Unity cannot auto-simulate FixedUpdateNetwork(). Changing {nameof(_physicsAuthority)} to {PhysicsAuthorities.Auto}.");
        ...
        /// <summary>
        ///     Restore sauto-simulate setting of the associated physics engine to its original value prior to any
        ///     <see cref="OverrideAutoSimulate" /> method calls.
        /// </summary>
        ...
        protected virtual void Startup()
        {
            ...
            _physicsAuthority = _physicsAuthority == PhysicsAuthorities.Auto
                ? Runner.Config.PeerMode == NetworkProjectConfig.PeerModes.Single &&
                  (Runner.GameMode == GameMode.Shared || Runner.Mode == SimulationModes.Host) &&
                  _physicsTiming != PhysicsTimings.FixedUpdateNetwork
                    ? PhysicsAuthorities.Unity
                    : PhysicsAuthorities.Fusion
                : _physicsAuthority;

#if UNITY_EDITOR
            if (_physicsAuthority == PhysicsAuthorities.Unity &&
                Runner.Config.PeerMode == NetworkProjectConfig.PeerModes.Multiple)
                Debug.LogWarning(
                    $"{GetType().Name}.{nameof(_physicsAuthority)} setting is forcing Unity as the Physics Authority. However in Multi-Peer Mode your Physics Scenes will not simulate. Set to Auto.");
#endif
            ...
            if (++_enabledRunnersCount == 1) OverrideAutoSimulate(_physicsAuthority == PhysicsAuthorities.Unity);
        ...
        protected virtual void Shutdown()
        {
            ...
            if (PhysicsAuthority == PhysicsAuthorities.Fusion && --_enabledRunnersCount == 0) RestoreAutoSimulate();
        }
        ...
        public override void FixedUpdateNetwork()
        {
            ...
            if (_isInitialized == false) Startup();
            ...
            if (Runner.TryGetPhysicsInfo(out var info))
            {
            ...
            var isForward = Runner.IsForward;
        ...
        {
            if (DeltaTimeMultiplier <= 0) return;

            if (isForward || !ForwardOnly) DoSimulatePrimaryScene(deltaTime);
        ...
        {
            while (_onBeforeSimulateCallbacks.Count > 0) _onBeforeSimulateCallbacks.Dequeue().Invoke();
            OnBeforeSimulate?.Invoke();
            SimulatePrimaryScene(deltaTime);
            HasSimulatedThisTick = true;

            while (_onAfterSimulateCallbacks.Count > 0) _onAfterSimulateCallbacks.Dequeue().Invoke();
        ...
        // One-time callbacks
        private readonly Queue<Action> _onAfterSimulateCallbacks = new();
        private readonly Queue<Action> _onBeforeSimulateCallbacks = new();
